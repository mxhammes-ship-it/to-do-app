...
|---|---|---|
| `surface` | `rgba(255,255,255,0.04)` | Standard-Card |
| `surface2` | `rgba(255,255,255,0.07)` | Hover, sekundäre Pille |
| `surface3` | `rgba(255,255,255,0.10)` | Aktiver Sidebar-Item |
| `divider` | `rgba(255,255,255,0.07)` | Standard-Trenner |
| `divider2` | `rgba(255,255,255,0.12)` | Card-Border |
...
- Badge (gefüllter Accent-Pill) **nur für Inbox**. Counts (Grau) für alles andere.
...
Hover auf einer Row hebt den Background um **eine Stufe** (`surface → surface2`). Kein Bouncen, kein Schwellen, kein Lift. Wenn der User auf eine Reihe zeigt, soll er nur lesen.
...
- **Keine Animationen bei Erledigung** (kein Konfetti, kein Sound).
...
13. **Counts in Sidebar:** Badge nur für Inbox, sonst grauer Number-Text.

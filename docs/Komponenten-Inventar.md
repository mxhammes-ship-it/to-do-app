...
struct CheckCircle: View {
    ...
    var body: some View {
        Button { withAnimation(.microState) { checked.toggle() } } label: {
            ZStack {
                Circle()
                    .strokeBorder(checked ? accent : (faint ? .textQuaternary : .textTertiary),
                                  lineWidth: 1.5)
                    .background(Circle().fill(checked ? accent : .clear))
                if checked {
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 6) {
                ...
                if let label { Text(label).font(.system(size: 12.5, weight: .medium)) }
            }
            .padding(.horizontal, label == nil ? 0 : 12)
            .frame(minWidth: size, height: size)
            .foregroundStyle(primary ? .white : .textPrimary)
            .background(
                Capsule().fill(primary ? AnyShapeStyle(accent) : AnyShapeStyle(Color.surface(level: 2)))
            )
            .overlay(Capsule().strokeBorder(.white.opacity(primary ? 0 : 0.08), lineWidth: 0.5))
            .overlay(alignment: .top) { Capsule().stroke(.white.opacity(primary ? 0.18 : 0.08), lineWidth: 1).blendMode(.overlay).mask(Capsule().padding(.bottom, size-1)) }
...
| ID | Name | Schicht | SwiftUI-Datei (Vorschlag) |
|---|---|---|---|
| F1 | AuroraWallpaper | Foundation | `DesignSystem/Materials/AuroraWallpaper.swift` |
| F2 | GlassBackground | Foundation | `DesignSystem/Materials/GlassBackground.swift` |
| F3 | SurfaceCard | Foundation | `DesignSystem/Materials/SurfaceCard.swift` |
| A1 | AppIcon | Atom | `DesignSystem/Atoms/AppIcon.swift` |
| A2 | CheckCircle | Atom | `DesignSystem/Atoms/CheckCircle.swift` |
| A3 | Badge | Atom | `DesignSystem/Atoms/Badge.swift` |
| A4 | Chip | Atom | `DesignSystem/Atoms/Chip.swift` |
| A5 | GlassButton | Atom | `DesignSystem/Atoms/GlassButton.swift` |
| A6 | Kbd | Atom | `DesignSystem/Atoms/Kbd.swift` |
| A7 | SectionLabel | Atom | `DesignSystem/Atoms/SectionLabel.swift` |
| A8 | TrafficLights | Atom (native) | — (NSWindow standard) |
| M1 | SidebarItem | Molecule | `DesignSystem/Molecules/SidebarItem.swift` |
| M2 | SidebarSection | Molecule | `DesignSystem/Molecules/SidebarSection.swift` |
| M3 | TimelineRow | Molecule | `DesignSystem/Molecules/TimelineRow.swift` |
| M4 | InboxRow | Molecule | `DesignSystem/Molecules/InboxRow.swift` |
| M5 | FieldRow | Molecule | `DesignSystem/Molecules/FieldRow.swift` |
| M6 | AISummaryCard | Molecule | `DesignSystem/Molecules/AISummaryCard.swift` |
| M7 | ProgressCard | Molecule | `DesignSystem/Molecules/ProgressCard.swift` |
| M8 | AccountCard | Molecule | `DesignSystem/Molecules/AccountCard.swift` |
| M9 | NotificationCard | Molecule | `Features/Notification/NotificationCard.swift` |
| M10 | SnoozeMenu | Molecule | `Features/Notification/SnoozeMenu.swift` |
| M11 | Dock (mock) | — | — (nicht produktiv) |
| O1 | AppSidebar | Organism | `DesignSystem/Organisms/AppSidebar.swift` |
| O2 | AppToolbar | Organism | `DesignSystem/Organisms/AppToolbar.swift` |
| O3 | ExpandedInboxItem | Organism | `Features/Inbox/ExpandedInboxItem.swift` |
| O4 | NotificationStack | Organism | `Features/Notification/NotificationStack.swift` |
| O5 | QuickCapturePill | Organism | `Features/Capture/QuickCapturePill.swift` |
| L1 | TahoeWindow | Shell | — (via NavigationSplitView) |
